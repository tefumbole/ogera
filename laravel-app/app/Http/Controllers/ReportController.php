...
class ReportController extends Controller
{
    ...
    protected function buildBookingCalendarData($year, $month, $warehouse_id = 0, array $product_ids = [])
    {
        ...
        for ($day = 1; $day <= $number_of_day; $day++) {
            ...
            $query = BookingProduct::with(['booking.customer', 'product'])
                ->whereDate('start', '<=', $date)
                ->whereDate('end', '>=', $date);

            $excludedCategoryIds = BookingCategoryHelper::calendarExcludedCategoryIds();
            if (!empty($excludedCategoryIds)) {
                $query->whereHas('product', function ($productQuery) use ($excludedCategoryIds) {
                    $productQuery->whereNotIn('category_id', $excludedCategoryIds);
                });
            }
            ...
            if ($booked) {
                $booking_details[$date] = $items->map(function ($line) {
                    return [
                        ...
                        'start' => $line->start,
                        'end' => $line->end,
                        'payment_status' => optional($line->booking)->payment_status,
                    ];
                })->values()->all();
            }
        }
    ...
    public function categoryReport()
    {
        ...
        $end_date = date('y-m-d');
        $warehouse_id = 0;
        $all_categories = Category::where('is_active', true)->get();
        $lims_warehouse_list = Warehouse::where('is_active', true)->get();
        return view('report.category_report',compact('start_date', 'end_date', 'warehouse_id', 'lims_warehouse_list', 'all_categories'));
    }
    ...
    public function averageSale()
    {
        ...
        $lims_warehouse_list = Warehouse::where('is_active', true)->get();
        return view('report.average_report',compact('start_date', 'end_date', 'warehouse_id', 'lims_warehouse_list', 'lims_products_list'));
    }
    public function JE()
    {
        $start_date = date('y-m-01');
        ...
        $lims_products_list = Product::where('is_active', true)->get();
        $lims_warehouse_list = Warehouse::where('is_active', true)->get();
        return view('report.JE_report',compact('start_date', 'end_date', 'warehouse_id', 'lims_warehouse_list', 'lims_products_list'));
    }

    public function JEData(Request $request)
    {
        $request_data = $request->all();
        $start_date = $request_data['start_date'];
        $end_date = $request_data['end_date'];
        $prepared = $request_data['prepared'] ?? Auth::user()->name;
        $code = $request_data['code'] ?? 'xxxx';
        $page = $request_data['page'] ?? 0;
        ...
        if ($end_date && $start_date) {
            ...
            $data = Payment::with('accounts')->whereBetween('created_at', [$yesterday, $tomorrow]);

            if ($payment != 0) {
                $data = $data->where('paying_method', $payment_method);
            }
            $data = $data->get();

            return view('report.JE_report',compact('start_date', 'end_date', 'prepared', 'code', 'page', 'data', 'payment'));
        }

        return redirect()->back();
    }

    public function averageSaleData(Request $request)
    {
        $lims_products_list = Product::where('is_active', true)->get();
        $lims_warehouse_list = Warehouse::where('is_active', true)->get();
        $request_data = $request->all();
        ...
        $data = Product_Sale::groupBy('product_id')
        ...
        return view('report.average_report',compact('start_date', 'end_date', 'data', 'diff_date', 'lims_warehouse_list', 'lims_products_list', 'warehouse_id'));
        }
    ...
    public function categoryReportData(Request $request)
    {
        ...
        $all_categories = Category::where('is_active', true)->get();
        $lims_category_list = $category_id == 0 ? Category::where('is_active', true)->get() : Category::where('id', $category_id)->get();
        $warehouse_id = $data['warehouse_id'];
        $yesterday = date('Y-m-d', strtotime($start_date));
        $tomorrow = date('Y-m-d', strtotime($end_date . "+1 days"));
        $warehouse = null;

        return view('report.category_report',compact('all_categories', 'warehouse', 'yesterday', 'tomorrow', 'start_date', 'end_date', 'category_id', 'lims_warehouse_list', 'data', 'lims_category_list', 'category_id', 'warehouse_id'));
    ...
    public function customerReport(Request $request)
    {
        ...
        return view('report.customer_report', compact('lims_sale_data','customer_id', 'start_date', 'end_date', 'lims_product_sale_data', 'lims_payment_data', 'lims_customer_list', 'lims_quotation_data', 'lims_product_quotation_data', 'lims_return_data', 'lims_product_return_data', 'customer_name'));
    }
    ...
    public function DueCustomerReport() {
        $lims_customer_all = Customer::whereHas('sales', function ($query) {
            $query->where('payment_status', '!=', 4);
            $query->where('paid_amount', '<', 'grand_total');
        })->orWhere('deposit', '<', 0)->get();
        $depositors = User::where('role_id', 14)->where('is_active', true)->get();

        return view('report.patient_due_report', compact('lims_customer_all', 'depositors'));
    }
}
