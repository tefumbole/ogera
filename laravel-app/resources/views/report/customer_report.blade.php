        ...
        buttons: [
            ...
            },
            {
                ...
                customize: function(win) {
                    $(win.document.body).prepend('<h1>Customer Report: {{ $start_date }} - {{ $end_date }}</h1>');
                    {{--$(win.document.body).prepend('<h3>Customer Name: {{ $customer_name }}</h3>');--}}
                    $(win.document.body).prepend('<img src="{{url('public/logo', $general_setting->email_water_mark)}}" style="width: 20%; position: absolute; top: 20%; right: 440px; opacity: 0.3;">');
                    $(win.document.body).prepend('<img src="{{url('public/logo', $general_setting->email_header)}}" width="100%">');
                    $(win.document.body).append('<img src="{{url('public/logo', $general_setting->email_footer)}}" width="100%">');
                },
                exportOptions: {
                    columns: ':visible:Not(.not-exported-sale)',
                    rows: ':visible'
