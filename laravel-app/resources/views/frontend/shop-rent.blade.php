                    ...
                    <div class="col-lg-12 order-first order-lg-last">
                        <div class="header-search">
                            <div class="box-header-search">
                                <form class="form-search" method="get" action="{{ route('frontend.product.search.rent') }}">
                                    @csrf
                                    <div class="box-keysearch">
                                        <label class="control-label color-danger">Search Rent Item</label>
                                        <input class="form-control font-xs" type="text" name="search" value="" placeholder="Search Rent Item">
                                    </div>
                                </form>
                            </div>
...
{{--                                                <span class="label bg-brand-2">-17%</span>--}}
                                            ...
                                            <div class="info-right"><a class="font-xs color-gray-500" href="">{{ @$product->category->name }}</a><br>
                                                ...
                                                } elseif ($product->rent_price_per_month != 0) {
                                                    $rent_price = $product->rent_price_per_month;
                                                }
                                                @endphp
                                                <div class="price-info"><strong class="font-lg-bold color-brand-3 price-main">
                                                        {{ number_format($rent_price, 2) }} {{ $currency->code }}</strong></div>
                                                @if($product->qty > 0 || $product->type != 'standard')
                                                    @if($product->rent_price_per_hour > 0 || $product->rent_price_per_day > 0 || $product->rent_price_per_month > 0)
                                                        <div class="mt-20 box-btn-cart"><a class="btn btn-cart" onclick="addtoRentcart({{ $product->id }},'/addToRentCart')">Book / Rent</a></div>
                                                    @endif
                                                @else
