...
@endif
                ...
                <div id="receipt-data">
                    ...
                    @else
                    @endif
                    @if((int) $lims_sale_data->quotation_status === \App\Quotation::STATUS_APPROVED && $lims_sale_data->clientSignatureUrl())
                        <div style="margin-top:18px;max-width:280px;">
                            <div style="font-size:11px;font-weight:bold;text-transform:uppercase;letter-spacing:.6px;">Approved &amp; signed by client</div>
                            <img src="{{ $lims_sale_data->clientSignatureUrl() }}" alt="Client signature" style="max-height:70px;max-width:100%;display:block;margin:4px 0;">
                            <div style="border-top:1px solid #999;padding-top:3px;font-weight:bold;">{{ @$lims_customer_data->name }}</div>
                            @if($lims_sale_data->client_signed_at)
                                <div style="font-size:11px;color:#666;">Signed {{ $lims_sale_data->client_signed_at->format('d-m-Y H:i') }}</div>
                            @endif
                        </div>
                    @endif
